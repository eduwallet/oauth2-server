...
func (h *RegistrationHandler) HandleRegistration(w http.ResponseWriter, r *http.Request) {
	h.log.Printf("🔍 [REGISTRATION] Starting client registration request")
	h.log.Printf("🔍 [REGISTRATION] Config available: %v", h.config != nil)
	if h.config != nil {
		h.log.Printf("🔍 [REGISTRATION] PrivilegedClientID: '%s'", h.config.Security.PrivilegedClientID)
	}
	h.log.Printf("🔍 [REGISTRATION] Method: %s, URL: %s", r.Method, r.URL.Path)
	...
	h.log.Printf("🔍 [REGISTRATION] Creating fosite.DefaultClient")
	...
	h.log.Printf("✅ [REGISTRATION] fosite.DefaultClient created successfully")
	h.log.Printf("🔍 [REGISTRATION] Client details: ID=%s, Public=%v, GrantTypes=%v, ResponseTypes=%v, Scopes=%v, Audience=%v",
