...
// ClientManager implements fosite.ClientManager
type ClientManager struct {
	clients map[string]*SimpleClient
	mutex   sync.RWMutex
}

// NewClientManager creates a new simple client manager
...
func (m *ClientManager) RegisterClient(req ClientRegistrationRequest) (*SimpleClient, error) {
	m.mutex.Lock()
...
func (m *ClientManager) LoadClientsFromConfig(configClients []config.ClientConfig) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
